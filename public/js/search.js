var word = window.parent.document.getElementById('search_word').value;
var style = '单曲';
construct(style);
function construct(style)
{
	$.ajax({
		type:'get',
	    dataType:'json',
	    url:'/index/index/doSearch',
	    async:true,
	    data:{value:word,style:style},
	    success:set
	});
}
...
function create(data, style, id)
{
	...
	if (data != '') {
		...
		if (style == '歌单') {
			for (var i = 0; i < data.length; i++) {
				$('.search_result').append('<div class="col-xs-6 col-sm-4 col-md-3 col-lg-2"><div class="item"><div class="pos-rlt"><div class="top"><span class="pull-right m-t-sm m-r-sm badge bg-info">'+data[i].sum+'</span></div><div class="item-overlay opacity r r-2x bg-black"><div class="center text-center m-t-n"><a href="/index/index/listSong?id='+data[i].id+'"><i class="fa fa-folder-open-o i-2x"></i></a></div></div><a href="#"><img src="'+data[i].poster+'" alt="" class="r r-2x img-full"></a></div><div class="padder-v"><a href="#" class="text-ellipsis">'+data[i].list+'</a><a href="#" class="text-ellipsis text-xs text-muted">'+data[i].author+'</a></div></div></div>')
			}
		}
    if (style == '歌手') {
      for (var i = 0; i < data.length; i++) {
        $('.search_result').append('<div class="panel panel-default"><div class="panel-heading" style="background: rgba(208,209,201,0.5);"><div class="media"><span class="pull-left thumb-sm"><a href="/index/index/singer?mid='+data[i].id+'"><img src="'+data[i].pic+'" alt="..." class="img-circle"></a></span><div class="media-body"><div><a href="/index/index/singer?mid='+data[i].id+'">'+data[i].singer+'</a></div><small class="text-muted">'+data[i].introduce+'</small></div></div></div></div></div>')
      }
    }
	} else {
			$('.search_result').append('<p class="text-center">您搜索 '+style+' 的结果为空...</p>');
			$('#fenye').html('')
	}
    ...
    methods:{
      msgListView(curPage){
        ...
        $.ajax({
          ...
          dataType:'json',
          url:'/index/index/doSearch',
          data:{value:word,style:style,page:curPage},
          async:true,
          success:set2
        });
      }
